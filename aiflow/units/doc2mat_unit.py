...
class Doc2MatUnit(Unit):

    def __init__(self, n_max_len, word2vec_model_url, *args, **kwargs):
    ...
    def execute(self, **kwargs):
        ...
        if not vectors:
            return np.zeros((self.n_max_len, self.n_ouput_dim))

        mat = np.concatenate(vectors).reshape(len(vectors), -1)
